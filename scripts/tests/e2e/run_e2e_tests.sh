...
# End-to-End test runner for full workflow validation
TEST_DIR="$(dirname "$0")/.."
OUTPUT_DIR="${TEST_DIR}/results/e2e"
mkdir -p "${OUTPUT_DIR}"
...
check_dependencies() {
  if ! command -v curl &> /dev/null; then
    echo "Error: curl is required for E2E tests" >&2
    exit 1
  fi
}
...
check_application_health() {
  ...
  if ! curl -s --retry 5 --retry-delay 3 "${health_endpoint}" | grep -q "ok"; then
    echo "Error: Application is not healthy at ${health_endpoint}" >&2
    exit 1
  fi
}

# Find all E2E test files (pattern: *_e2e.py or *.e2e.js)
while IFS= read -r -d $'\0' file; do
  test_files+=("$file")
done < <(find "${TEST_DIR}" -type f \( -name "*_e2e.py" -o -name "*.e2e.js" \) -print0)
...
# Function to run JavaScript E2E tests
run_javascript_test() {
  local file="$1"
...
for test_file in "${test_files[@]}"; do
  echo "Running E2E tests in: ${test_file}"
